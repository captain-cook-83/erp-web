<template>
    <div v-loading="loading" element-loading-background="rgba(255, 255, 255, 0.65)">
        <el-form v-if="model.id == null" :model="model" :rules="rules" ref="form">
            <el-form-item  label="关联合同" label-width="80px" prop="contractId">
                <el-popover v-if="model.id == null" placement="bottom" :offset="-40" width="950" v-model="contractSeletorSHown">
                    <resource-selector v-if="model.id == null" resourceURL="/contracts" :filters="contractFilters" :tableColumes="contractSelectorColumes" 
                    ...
                    <el-input slot="reference" v-model="model.contractTitle" suffix-icon='el-icon-search' placeholder="请选择关联合同" 
                        :validate-event="false" :clearable="true" tabindex="1" @keyup.enter.native="onContractSearch" @clear="onContractClear" 
                        @change="onContractTitleChange" @blur="onContractBlur"/>
                </el-popover>
            </el-form-item>
            ...
            <el-form-item label="发生期间" label-width="80px" prop="periodDates">
                <el-date-picker v-if="contractItem == null" v-model="model.periodDates" type="daterange" align="right" unlink-panels :picker-options="pickerDateOptions"
                    range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="onPeriodDatesChanged"/>
                <el-tooltip v-else content="月度记录已存在，发生期间无法修改" placement="left" effect="light">
                    <el-date-picker v-model="model.periodDates" type="daterange" align="right" unlink-panels
                        :readonly="true" prefix-icon="-" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"/>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="核算月份" label-width="80px" prop="accountingDate">
                <el-date-picker v-if="contractItem == null" v-model="model.accountingDate" align="right" placeholder="请选择核算月份" :editable="false" :clearable="true"
                    :validate-event="false" :picker-options="pickerDateOptions" type="month" tabindex="2"/>
                <el-tooltip v-else content="月度记录已存在，核算月份无法修改" placement="left" effect="light">
                    <el-date-picker v-model="model.accountingDate" align="right" placeholder="请选择核算月份" :editable="false"
                        :readonly="true" prefix-icon="-" type="month"/>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="阶段" label-width="80px" prop="type">
                <el-select v-model="model.type" placeholder="请选择阶段">
                    <el-option v-for="option in typeOptions" :key="option" :label="typeEnum[option]" :value="option"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="核算金额" label-width="80px" prop="amount">
                <!-- <el-input-number v-model="model.amount" controls-position="right" 
        ...
        <el-form v-else :model="model" :rules="rules" ref="form">
            <el-form-item  label="关联合同" label-width="80px" prop="contractId">
                <el-tooltip content="关联合同无法修改" placement="left" effect="light">
                    <el-input v-model="model.contractTitle" :readonly="true"/>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="备注" label-width="80px" prop="remarks">
                <el-input v-model="model.remarks" :autosize="{ minRows: 1, maxRows: 2}" tabindex="6"/>
            ...
            <el-form-item label="发生期间" label-width="80px" prop="periodDates">
                <el-tooltip content="发生期间无法修改" placement="left" effect="light">
                    <el-date-picker v-model="model.periodDates" type="daterange" align="right" unlink-panels
                        :readonly="true" prefix-icon="-" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"/>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="核算月份" label-width="80px" prop="accountingDate">
                <el-tooltip content="核算月份无法修改" placement="left" effect="light">
                    <el-date-picker v-model="model.accountingDate" align="right" placeholder="请选择核算月份" :editable="false"
                        :readonly="true" prefix-icon="-" type="month"/>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="填报阶段" label-width="80px" prop="type">
                <el-tooltip content="填报阶段无法修改" placement="left" effect="light">
                    <div style="border: 1px solid #DDD; border-radius: 4px; padding-left: 15px; width: 203px">
                        <enum-formater slot="prepend" :value="model.type" :formater="typeEnum"></enum-formater>
                    </div>
                </el-tooltip>
            </el-form-item>
            <el-form-item label="核算金额" label-width="80px" prop="amount">
        ...
        <div class="bills-container">
            <div>
                <el-popover v-model="createDialogShown" placement="bottom-end" width="900">
                    <accounting-bill-form v-once :columes="billColumes" :border="true" :fixed-type="true" ref="createDialog"
                ...
                <el-popover v-model="modifyDialogShown" placement="bottom-end" width="900">
                    <accounting-bill-form v-once :columes="billColumes" :border="true" :fixed-type="true" ref="modifyDialog"
...
export default {
    ...
    computed: {
        typeOptions () {
            return Utils.keys(this.typeEnum);
        }
    },
    methods: {
        setModel: function (model) {
            ...
            if (model.id) {
                this.loading = true;
                Axios.get('/contractItemSources/' + model.id + "/bills").then(response => {
        ...
        getModel: function () {
            ...
            this.billsData.forEach(item => {
                if (item._s) {
                    ...
                    item.type = this.model.type;    // 固定类型
        ...
        checkCurrentContractItem: function () {
            this.loading = true;
            Axios.get('/contractItems/' + this.model.contractItemId).then(response => {
                let contractItem = response.data;
                if (contractItem) {
                    // Vue.set(this.model, 'startDate', contractItem.startDate);
                    // Vue.set(this.model, 'endDate', contractItem.endDate);
                    // Vue.set(this.model, 'accountingDate', contractItem.accountingDate);
        ...
        checkDerivedContractItem: function () {
            this.loading = true;
            Axios.get('/contractItems/derived?contractId=' + this.model.contractId).then(response => {
                let contractItem = response.data;
                if (contractItem) {
                    Vue.set(this.model, 'contractItemId', contractItem.id);
                    Vue.set(this.model, 'startDate', contractItem.startDate);
                    Vue.set(this.model, 'endDate', contractItem.endDate);
                    Vue.set(this.model, 'accountingDate', contractItem.accountingDate);
                    Vue.set(this.model, 'periodDates', contractItem.periodDates);
